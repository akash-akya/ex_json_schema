defmodule ExJsonSchema.Validator.Error do
  ...
  @spec format(ExJsonSchema.Validator.errors() | {:error, ExJsonSchema.Validator.errors()} | :ok) ::
  ...
  defimpl String.Chars, for: AllOf do
    def to_string(%AllOf{invalid: invalid}) do
      "Expected all of the schemata to match, but the schemata at the following indexes did not: " <> Enum.map_join(invalid, ", ", &Kernel.to_string/1)
    end
  end

  defmodule AnyOf, do: defstruct [:invalid]
  defimpl String.Chars, for: AnyOf do
    def to_string(%AnyOf{invalid: invalid}) do
      "Expected any of the schemata to match but none did: "  <> Enum.map_join(invalid, ", ", &Kernel.to_string/1)
    end
  end

  defmodule OneOf, do: defstruct [:valid_indices, :invalid]
  defimpl String.Chars, for: OneOf do
    def to_string(%OneOf{invalid: invalid, valid_indices: valid_indices}) do
      if length(valid_indices) > 1 do
        ...
        "Expected exactly one of the schemata to match, but none did: " <> Enum.map_join(invalid, ", ", &Kernel.to_string/1)
      end
    end
  end
  ...
  defimpl String.Chars, for: InvalidAtIndex do
    def to_string(%InvalidAtIndex{index: index, errors: errors}) do

      "Index #{index}: (#{Enum.map_join(Error.format(errors), ", ", fn {error, path} -> "{#{error}, #{path}}" end)})"
    end
  end
  ...
  defimpl String.Chars, for: AdditionalItems do
    def to_string(%AdditionalItems{}) do
  ...
  defimpl String.Chars, for: Format do
    def to_string(%Format{expected: expected}) do
